...
The `apify` CLI must be available in the session (`npm install -g apify-cli`,
authenticated via `apify login` or an environment-seeded `APIFY_TOKEN`):

- `apify actors call` — run an Actor
- `apify actors info --input` — fetch an Actor's input schema
- `apify api /v2/datasets/<id>/items` — fetch run output
