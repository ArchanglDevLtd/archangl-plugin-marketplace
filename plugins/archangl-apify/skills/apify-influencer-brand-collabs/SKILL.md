...
description: |
  ...
  **Triggers:**
  ...
  data, so don't ask the user to declare it. Requires the apify CLI.
author: Natasha Lekh
...
Call each via `apify actors call <actor-id> --input '<json>'`. Use `apify actors info <actor-id> --input`
