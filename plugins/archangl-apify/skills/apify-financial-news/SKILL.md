...
- Apify access — the `apify` CLI (`npm install -g apify-cli` and `apify login`, or `APIFY_TOKEN` seeded in the environment). The CLI is the only transport.
...
```
Task Progress:
- [ ] Step 0: Verify prerequisites — try `apify --version && apify info`; if unavailable, stop and tell the user to install the apify CLI. Also verify: `python3 -c "from readability import Document; print('OK')"` (install: `pip install readability-lxml lxml`)
