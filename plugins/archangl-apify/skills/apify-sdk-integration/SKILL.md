...
Before writing integration code, find the Actor that fits the user's needs:
- `apify actors ls --search "<keyword>"` — search the Apify Store from the CLI
- `apify actors info <actor-id> --input` — get the Actor's input schema
- Browse https://apify.com/store; append `.md` to any Actor's Store URL to get its docs in markdown.
...
- **Handle Actor-specific input:** Every Actor has its own input schema. Use `apify actors info <actor-id> --input` or append `.md` to the Actor's Store URL to get the schema before constructing input.
