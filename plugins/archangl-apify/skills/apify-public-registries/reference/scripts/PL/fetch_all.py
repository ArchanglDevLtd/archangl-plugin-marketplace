...
Sources:
  ...
  financials   — Finanční výkazy z eKRS (via Apify actor minute_contest/poland-krs-financial-scraper).
                 ...
                 Vyžaduje apify CLI s APIFY_TOKEN v prostředí.
"""
...
# Note: This requires the apify CLI. Run separately:
#   apify actors call minute_contest/poland-krs-financial-scraper --input '{"krs":"0000536554"}' 
...
def fetch_krs_financials_instructions():
    ...
    for krs, info in companies.items():
        cmd = (
            f"apify actors call minute_contest/poland-krs-financial-scraper "
            f"--input '{{\"krs\":\"{krs}\"}}'"
        )
        cmds.append(cmd)
