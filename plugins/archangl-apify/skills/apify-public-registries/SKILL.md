...
- Apify access (only for DE, UK, PL financials, CZ justice.cz, RO fallback) — the `apify` CLI (`npm install -g apify-cli` and `apify login`) plus `APIFY_TOKEN` seeded in the environment (the bundled fetch scripts call the Apify REST API with it). The CLI/REST path is the only transport.
...
```
Task Progress:
- [ ] Step 0: Verify prerequisites — run `python3 --version` (required); for Apify-dependent registries: try `apify --version && apify info` and check `APIFY_TOKEN` is set; if not, stop and tell the user to install the apify CLI / seed the token
...
| **DE** | `python3 reference/scripts/DE/fetch_all.py keyword <name>` | Company name | Apify (REST via APIFY_TOKEN) |
| **UK** | `python3 reference/scripts/UK/fetch_all.py search <name>` | Company name | Apify (REST via APIFY_TOKEN) |
...
Note: DE, UK, and RO scripts call the Apify REST API internally using `APIFY_TOKEN` from the environment.
...
- **DE + UK scripts call the Apify REST API** — require `APIFY_TOKEN` in the environment.
