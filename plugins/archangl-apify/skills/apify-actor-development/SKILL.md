...
**When to use Standby mode:** Use Standby when the Actor must handle interactive, real-time HTTP requests — API endpoints, webhook receivers, real-time data lookups, tool servers, or scraping APIs serving on-demand single-URL requests.
...
For debugging Actors that interact with the web, drive a real browser with a
Playwright script (`npx playwright`, Chromium) to inspect pages, capture
selectors, and reproduce issues — run it via Bash rather than adding any
