...
- **Owner note (degraded mode active):** this marketplace ships the skill without its upstream Ahrefs scoring dependency (the banned server transport is not permitted, and no Ahrefs CLI/key is wired). Step 5 is skipped per the skill's own fallback: authority columns and `Prospect Tier` render as `"-"`. Tier scoring can return if the owner later seeds an Ahrefs REST key and ports Step 5.
...
# 3. (Skipped in degraded mode — no Ahrefs source. If enrichment data exists at
#    {base}_ahrefs_domain.json + {base}_ahrefs_page.json, enrich as upstream did.)
...
- [ ] Step 5: SKIPPED (degraded mode) — tiers render as "-"; see Step 5 note
...
6. **Competitors** — anyone in the user's product category who would publish a "ours vs theirs" comparison page on their own site. Frame the ask this way explicitly: *"List every company that would write an X-vs-YourBrand comparison page. These won't link to you no matter what — small competitors count too."* Encourage 10+ entries; most users default to listing 3–5 obvious ones and miss the long tail. Mapped to `competitorDomains` on the Actor and reused in Steps 6 (adversarial-mention skip) and 7 (`Links to competitor` Why-tag).
   ...
   Proceed with the user-supplied competitor list only (upstream's optional Ahrefs organic-competitor auto-pull is not available in this build).
...
### Step 5: Prospect Tier (degraded mode — skipped)

Upstream enriched every surviving domain with Ahrefs authority metrics
(domain rating, page traffic, referring domains) and computed a `Prospect
Tier`. This build has no Ahrefs source, so per the skill's own fallback rule:
set `Domain DR`, `Page Traffic`, `Referring Domains`, and `Prospect Tier` to
`"-"` for every row, and add a one-line note in the output header explaining
that tiers were not computed. Do not fabricate values, and do not attempt any
other scoring source. Continue directly to Step 6.
...
| Ahrefs source unavailable (always true in this build) | Skip Step 5. Set `Domain DR`, `Page Traffic`, `Referring Domains`, `Prospect Tier` to `"-"` and add a one-line note in the output header explaining tiers were not computed. Continue with the rest of the workflow. |
