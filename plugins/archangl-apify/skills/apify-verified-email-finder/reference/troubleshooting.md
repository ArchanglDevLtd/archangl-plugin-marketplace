...
- **`APIFY_TOKEN not found`** (script path only) — Create `.env` with `APIFY_TOKEN=...` (or rely on the environment-seeded `APIFY_TOKEN`); get the token from `https://console.apify.com/account/integrations`.
