...
The skill runs on the script path — the only transport. Requires:
...
**CLI path (equivalent to the script):**

`apify actors call <actor-id> --input '<json from Step 3>' --timeout 1800 --memory 4096` with one of the three Actor IDs (`compass/crawler-google-places`, `apify/google-search-scraper`, `vdrmota/contact-info-scraper`). The call returns `runId` and `datasetId` — capture both for the `run_metadata.json` sidecar.
...
Pull the dataset:
- **CLI path:** fetch items via `apify api "/v2/datasets/<id>/items?fields=title,searchString,countryCode,city,address,phone,website,leadsEnrichment&clean=true"` (or the equivalent `curl https://api.apify.com/v2/datasets/<id>/items?...` with `Authorization: Bearer $APIFY_TOKEN`) and pipe through `jq`.
