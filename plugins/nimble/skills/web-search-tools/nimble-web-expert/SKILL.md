...
- **Verify the CLI BEFORE working — don't fire a data call and react to the error.** `nimble --version` + `NIMBLE_API_KEY` set confirms the CLI is ready.
- **No working CLI → stop.** Do not fall back to WebFetch, WebSearch, curl, or `dangerouslyDisableSandbox`. Follow the install/key flow in `rules/setup.md` and wait for the user.
...
The `nimble` CLI is the only transport. Confirm it once at session start and don't re-probe on every command.
...
- **Not ready** → load `rules/setup.md` for the install/key flow (`npm i -g @nimble-way/nimble-cli`, then seed `NIMBLE_API_KEY` into the environment), then stop and wait. **Never substitute WebFetch, WebSearch, curl, or any other tool for Nimble operations.**
