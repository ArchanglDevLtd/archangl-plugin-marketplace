...
The `nimble` CLI is the only transport (it exposes the full surface area — every flag, batch ops, file I/O). Confirm it once at session start and don't re-probe on every command:
...
- **Not ready** → load `rules/setup.md` for the install/key flow (`npm i -g @nimble-way/nimble-cli`, then seed `NIMBLE_API_KEY` into the environment), then stop and wait. Never substitute WebFetch, WebSearch, curl, or any other tool.
...
If `nimble agent generate` (fresh or `--from-agent`) cannot produce a working agent because the site's data structure is unknown (wrong selectors, missing XHR patterns, unexpected JS rendering):
...
**Step 3 — Use the report** to retry agent generation: pass the selector/XHR context and suggested extract command in the refinement prompt to `nimble agent generate`.
...
The Task agent uses CLI commands via Bash.
...
**CRITICAL: Use the nimble CLI for all operations. NEVER use WebSearch, WebFetch, curl, or wget.**
```
...
**Use the Nimble CLI to inspect agent schemas (via Bash):**
```bash
nimble agent get --template-name <agent_name>
```
This returns the full input/output schema for the agent.

**CRITICAL: Use CLI (Bash) for all Nimble operations. NEVER use WebSearch, WebFetch, curl, or wget.**
...
Do NOT use WebSearch, WebFetch, bash curl, or any other search/fetch method.
```
...
Load reference files **only during large-scale script generation (Step 2B)** or agent creation (Step 3). Do NOT load these for interactive runs (Step 2A) — the CLI command registry above is sufficient.
...
- **`references/agent-api-reference.md`** — CLI and REST API reference plus input parameter mapping.
...
- **All operations use the CLI (Bash).** There is no fallback transport.
