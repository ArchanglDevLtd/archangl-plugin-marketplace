...
Follow the CLI preflight from `references/nimble-playbook.md` — confirm the `nimble` CLI and `NIMBLE_API_KEY` are available, then run the standard preflight calls (date calc, today, profile, memory index) in parallel.

From the results:
- CLI missing or API key unset → `references/profile-and-onboarding.md`, stop
- Tag all `nimble` CLI calls: `nimble --client-source skill-meeting-prep <subcommand>`.
...
is available — either a calendar tool (look for `list_events` in the tool list)
