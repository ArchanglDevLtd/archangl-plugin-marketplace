...
### CLI preflight (run once per session)

The `nimble` CLI is the only transport. Confirm it once at the start of the
session and stick with it; don't re-probe on every command.

| Check | If it works | If it fails |
|---|---|---|
| `nimble --version` (>= 0.12.0) | CLI is installed — use Bash `nimble ...` commands | Stop — guide install (below) |
| `NIMBLE_API_KEY` is set in the environment | Authenticated — proceed | Stop — guide key setup (below) |

### CLI or key missing

If either check fails, surface this hint verbatim and stop — do **not** fall back
to WebFetch, WebSearch, curl, or any other tool, and do **not** guess at data:

> The Nimble CLI isn't ready in this environment:
>
> **If `nimble` isn't installed:**
> ```
> npm i -g @nimble-way/nimble-cli
> ```
>
> **If `NIMBLE_API_KEY` isn't set:** seed it into the environment (env var or the
> environment's startup configuration) — don't paste the key into this
> conversation. See `references/profile-and-onboarding.md` for the full flow.
>
> Once both are in place, re-run your request and I'll continue.

### Standard preflight (run in parallel after the CLI preflight)
...
Don't skip the CLI preflight — running `nimble` commands before confirming the CLI
and key wastes a turn and confuses the user.
...
Add `--client-source skill-{name}` as the global flag on every `nimble`
...
If `nimble --version` returns "command not found", stop and follow the "CLI or key
missing" guidance in the Preflight Pattern above. There is no fallback transport.
