...
All queries below are `nimble` CLI commands (`nimble search …`), run per the CLI preflight in `references/nimble-playbook.md`. See the "Nimble social search configuration" section below for the date-window flags.
...
For social media sweeps, use `--focus social` — Nimble's social mode surfaces posts and threads more effectively than a plain web search. Apply the time window via `--start-date`/`--end-date` on every call. Use `--search-depth lite` for discovery passes and `--search-depth deep` to pull full thread content on high-signal hits.
