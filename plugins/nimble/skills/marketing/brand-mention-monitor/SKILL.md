...
Follow the CLI preflight from `references/nimble-playbook.md`: confirm the `nimble` CLI and `NIMBLE_API_KEY` are available, then run the parallel preflight calls (date, profile, memory index) simultaneously. Tag every Nimble CLI call: `nimble --client-source skill-brand-mention-monitor <subcommand>`.
