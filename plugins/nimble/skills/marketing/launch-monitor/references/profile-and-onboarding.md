...
The CLI preflight in `nimble-playbook.md` confirms the `nimble` CLI and
`NIMBLE_API_KEY` are ready. This section covers the install/upgrade/auth flow
when they aren't.
...
The `nimble` CLI is the only transport — install it directly; it exposes the full
Nimble surface area:
...
The CLI authenticates with the `NIMBLE_API_KEY` environment variable. If it isn't
set, ask the user to seed it into the environment (env var or the environment's
startup configuration) — never ask them to paste the key into the conversation.
