...
Follow the CLI preflight from `references/nimble-playbook.md`: confirm the `nimble` CLI and `NIMBLE_API_KEY` are available, then run the parallel preflight calls (date, profile, memory index) simultaneously. Tag every Nimble CLI call: `nimble --client-source skill-launch-monitor <subcommand>`.
...
- `https://techcrunch.com/2026/06/11/nimble-cli-connector-launch`
