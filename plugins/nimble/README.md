...
**This marketplace (vendored snapshot):**

```
/plugin install nimble@archangl-plugin-marketplace
```

The plugin ships skills, agents, and commands only. All Nimble operations run
through the `nimble` CLI, authenticated by the `NIMBLE_API_KEY` environment
variable (step 2 above).
...
| Aspect | Claude Code | npx skills |
| ------ | ----------- | ---------- |
| Plugin config | `.claude-plugin/` | N/A (reads `skills/`) |
| Rules | `rules/*.md` | N/A |
