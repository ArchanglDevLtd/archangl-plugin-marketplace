...
See `.claude-plugin/plugin.json` for the full list of published skill directories.
...
Verticals are just grouping folders — add new ones freely. `.claude-plugin/plugin.json` lists vertical directories explicitly. Update it when adding or removing verticals or agents.
...
The plugin manifest lives in `.claude-plugin/plugin.json`. It declares which `skills/` directories and `agents/` files are included. Update it when adding or removing a skill.

This is a vendored snapshot inside the archangl marketplace: the manifest carries no `version` (the marketplace repo's commits drive updates — see `SNAPSHOT.md`), so skip upstream's version-bump ritual. Skill-level `metadata.version` fields are upstream artifacts; leave them as snapshotted.
