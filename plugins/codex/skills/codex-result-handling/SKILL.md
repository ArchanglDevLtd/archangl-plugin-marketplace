...
> in `codex-cli-runtime`: ask the owner which model to use, recommend from a
> run-time grounded evaluation (installed `codex` CLI first, latest releases if
> unsure), and recommend a reasoning-effort level.
