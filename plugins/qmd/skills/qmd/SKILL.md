...
license: MIT
compatibility: Requires the qmd CLI. Install via `npm install -g @tobilu/qmd`.
metadata:
  author: tobi
  version: "2.2.0"
...
For the best results, prefer structured multi-line query documents with
`qmd query` — combine typed sub-queries (the first gets 2x weight in fusion):

```bash
qmd query 'lex: cockpit OKR Goodhart
vec: data informed not metric driven product judgment
hyde: A concept note explains that metrics are useful as instruments, but leaders should not let OKRs or dashboards replace judgment.' \
  -c concepts -n 10 --intent "Find the concept note about using metrics as instruments without becoming metric-driven."
```

Full query-document grammar: `docs/SYNTAX.md` in this plugin.
...
Install the CLI: `npm install -g @tobilu/qmd` (in the owner's environments this
is handled by the environment startup configuration). All access is through
`qmd` CLI commands — there is no other transport in this plugin.
